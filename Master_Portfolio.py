...
WHERE AccountingMonth BETWEEN @StartDate AND @EndDate;"""
    ...
    # File path
    #file_path = r"C:\Development\PowerBI\Master_Dash\Portfolio_sql.csv"
    ...
    print(f"SQL script read into pandas dataframe in {elapsed_time:.2f} seconds")

    # Define index and value columns
    index_columns = [
        'AccountingMonth', 'MarketName', 'StoreName', 'Department', 'StyleName',
        'FuelType', 'Make', 'Model', 'Trim', 'Hyperion', 'Loaner', 'AllocationGroup',
        'Status', 'ExService', 'Table_Source'
    ]

    value_columns = [
        'DaysInInventoryStore', 'MSRP_Adv', 'Balance', 'InvCount', 'SumSold',
        'SumFrontGross', 'SumFIGross', 'SumIncentiveGross', 'SumOVIGross'
    ]

    # Fill NaNs in value columns with 0
    df[value_columns] = df[value_columns].fillna(0)

    # Fill NaNs in index columns with a placeholder
    df[index_columns] = df[index_columns].fillna('NaN')
    ...
    pivot_df = df.pivot_table(
        ...
        values=[
            'Mileage', 'DaysInInventoryStore', 'Balance', 'InvCount', 'SumSold',
            'SumFrontGross', 'SumFIGross', 'SumIncentiveGross', 'SumOVIGross'
        ],
        aggfunc='sum'
    )

    # Expand index so that they are rows
    pivot_df = pivot_df.reset_index()


    ###FORMULAS
    # Extract Year from AccountingMonth (assuming format like '2025-08' or 'YYYY-MM')
    pivot_df['Year'] = pivot_df['AccountingMonth'].astype(str).str[:4]

    # Calculate AverageSellingPrice safely
    pivot_df['AverageSellingPrice'] = pivot_df.apply(
        lambda row: row['SumFrontGross'] / row['SumSold'] if row['SumSold'] != 0 else 0,
        axis=1
    )

    # Rename Columns to fit BI Model
    pivot_df.rename({'Make': 'Company', 'StoreName': 'Dealership', 'Model': 'Segment', 'MarketName': 'Region', 'AllocationGroup': 'Model', 'AccountingMonth': 'Sales Date', 
                     'SumSold': 'Units Sold', 'SumFrontGross': 'Revenue', 'AverageSellingPrice': 'ASP', 
                     'InvCount': 'Inventory Levels', 'DaysInInventoryStore': 'Days on Lot'})


    # Include/Exclude columns
    pivot_df = pivot_df[[
        'Model', 'Company', 'Year', 'Segment', 'Region', 'Dealership',
        'Sales Date', 'Units Sold', 'Revenue', 'Market Share', 'ASP', 'Customer Satisfaction', 'Inventory Levels', 'Days on Lot'
    ]]

    # Save to a new CSV if needed
    pivot_df.to_csv(r"C:\Development\PowerBI\Master_Dash\Portfolio_Pivot.csv", index=False)
    
    # End Timer
    end_time = time.time()
    elapsed_time = end_time - start_time
    print(f"Python script completed in {elapsed_time:.2f} seconds")
