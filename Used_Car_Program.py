...
################################################################################################################
    ...
    '''
    # for month end...need to remove extra data only update this month...
    Mkt_Data_query = """
    SELECT  [MONTH]
        ,[MARKET_NAME]
        ,[STORE]
...
################################################################################################################
    ...
    i1.Snapshot_Date between '{beginning_of_month}' and '{current_day}'
    ...
    if today.day >= 8:
        ...
        print("Today is before the 8th — skipping data update for mkt and associatedata.")
    ...
    print(f"Filtering out this for current month: {current_month}")
    current_year = reference_date.year
    print(f"Filtering out this for current year: {current_year}")

    Speed_to_Market_tab1 = Speed_to_Market_wb.sheets['Data1']

    # Read existing data (assuming headers in row 1, data from B2)
    data_range = Speed_to_Market_tab1.range('B1').expand('down').resize(None, 11)
    ...
    # Filter out rows for the current month in MTH column and current year in Year column that contain current year and month data...
    # we will adjust current mth/year data
    filtered_df = existing_df[
        ~(
            (existing_df['MTH'].astype(int) == current_month) &
            (existing_df['Year'].astype(int) == current_year)
        )
    ]
    ...
    '''repeat process for speed to market 2 where we replace the appended new data only on this month'''

    # Read existing data (assuming headers in row 1, data from B2)
    data_range = Speed_to_Market_tab2.range('B1').expand('down').resize(None, 10)
    existing_df = data_range.options(pd.DataFrame, header=1).value
    ...
    existing_df['SnapshotDate'] = pd.to_datetime(existing_df['SnapshotDate']).dt.date
    filtered_df = existing_df[existing_df['SnapshotDate'] != beginning_of_month]

    # Combine filtered existing data with new data
    updated_df = pd.concat([filtered_df, Speed_to_Market_df2], ignore_index=True)

    # Clear full range before re-writing cleaned data
    Speed_to_Market_tab2.range('B2:K500000').clear_contents()

    # Write back updated data
    Speed_to_Market_tab2.range('B2').options(index=False, header=False).value = updated_df
   ...
   # Save the file in the directory as a new name with todays date
    ...
    # Make copy of file to main location
    shutil.copyfile(r'C:\Users\BesadaG\OneDrive - AutoNation\PowerAutomate\Used_Car_Program\Used Car Program - Strategy Sheet.xlsx', new_file)
