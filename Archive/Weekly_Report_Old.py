...
################################################################################################################################
    ...
    # Formulas for Used_Pricing
    # 1. Age_Bucket
    # conditions = [
    #     Used_Pricing_df['AN_Age'].isna(),
    #     Used_Pricing_df['AN_Age'] < 16,
    #     Used_Pricing_df['AN_Age'] < 31,
    #     Used_Pricing_df['AN_Age'] < 46,
    #     Used_Pricing_df['AN_Age'] < 61,
    #     Used_Pricing_df['AN_Age'] < 91
    # ]
    ...
    # Used_Pricing_df['AgeBucket'] = np.select(conditions, choices, default='06:>90')

    # # 2. Franchise
    # Used_Pricing_df['Franchise_Type'] = np.where(
    #     Used_Pricing_df['StoreName'].str.startswith('AutoNation USA'),
    #     'Non-Franchise',
    #     'Franchise'
    # )

    # # 3. Discount
    # # Sort by VIN and SnapshotDate
    # Used_Pricing_df = Used_Pricing_df.sort_values(by=['VIN', 'SnapshotDate'])

    # # Calculate the previous price
    # Used_Pricing_df['PreviousPrice'] = Used_Pricing_df.groupby('VIN')['PriceAdj'].shift(1)

    # # Detect price changes
    # Used_Pricing_df['Price_Changed'] = Used_Pricing_df['PriceAdj'] != Used_Pricing_df['PreviousPrice']

    # # Label rows and calculate Discount
    # Used_Pricing_df['Label'] = Used_Pricing_df.apply(lambda row: 'Price_Changed' if row['Price_Changed'] else ('Day_Before' if row['Price_Changed'] == False and row['PreviousPrice'] != row['PriceAdj'] else 'Day_Before'), axis=1)
    # # Calculate discount   
    # Used_Pricing_df['Discount'] = Used_Pricing_df.apply(lambda row: row['PreviousPrice'] - row['PriceAdj'] if row['Label'] == 'Price_Changed' else 0, axis=1)

    # # Get Original days since last price change
    # # Sort by VIN and DaysSinceLPChange descending to simulate chronological order
    # Used_Pricing_df = Used_Pricing_df.sort_values(by=["VIN", "DaysSinceLPChange"], ascending=True).reset_index(drop=True)

    # # Logic to select the prior DaysSinceLPChange value where there was a price change and discount to take the prior dayssincelpchange number of days
    # for vin, group in Used_Pricing_df.groupby("VIN"):
    #     last_valid_day = None
    #     for idx in group.sort_values(by="DaysSinceLPChange", ascending=False).index:
    #         row = Used_Pricing_df.loc[idx]
    #         if row["Label"] == "Price_Changed":
    #             discount = row["Discount"]
    #             if pd.isna(discount) or discount == 0:
    #                 last_valid_day = row["DaysSinceLPChange"]
    #             elif last_valid_day is not None:
    #                 if last_valid_day is not None:
    #                     Used_Pricing_df.at[idx, "DaysSinceLPChange"] = last_valid_day
    #                     last_valid_day = None # Reset after applying to one valid discount
                        

    # # Remove duplicates, limit the dataframe to only the discount non-blanks and labels that say price_changed
    # Used_Pricing_df = Used_Pricing_df[
    #     (Used_Pricing_df['Discount'].notnull()) &
    #     (Used_Pricing_df['Discount'].astype(str).str.strip() != "") &
    #     (Used_Pricing_df['Label'] == "Price_Changed")
    # ]


    # # 4. Margin
    # Used_Pricing_df['Margin'] = Used_Pricing_df['PriceAdj'] - Used_Pricing_df['CostAdj']


    # # Convert SnapshotDate to datetime
    # Used_Pricing_df['SnapshotDate'] = pd.to_datetime(Used_Pricing_df['SnapshotDate'])

    # # Create 'Week' column based on the condition
    # Used_Pricing_df['Week'] = Used_Pricing_df['SnapshotDate'].apply(
    # lambda x: 'Week_2' if (today - x).days <= 7 else 'Week_1'
    # )
    ...
    '''Update Used Pricing'''
    # Used_Pricing_tab = Weekly_File_wb.sheets['Used_Pricing_Data']
    # Used_Pricing_tab.clear_contents()
    # Used_Pricing_tab.range('A1').options(index=False).value = Used_Pricing_df

    # Run Macro that creates target data in New_Sales_df
    # Open Dynamic_Daily_Sales.xlsb for macro use
